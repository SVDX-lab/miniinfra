"""第 02 期独立使用的 Qwen3 Byte-level BPE Tokenizer。

直接读取模型目录中的 tokenizer.json，不依赖 Transformers 或第一期代码。
只覆盖本期需要的文本编码、解码和 non-thinking 单轮聊天模板。
"""
...
class Qwen3Tokenizer:
    ...
    def apply_chat_template(self, messages, add_generation_prompt=True):
        ...
        if add_generation_prompt:
            # 与 Transformers 的 enable_thinking=False 模板保持一致。
