"""第 13 期 Paged KV Cache：支持同步与异步生命周期的两级 Block Pool。
...
class CPUPinnedPool:
    ...
    布局与 GPU Pool 完全一致（块主序）。资源预留、拷贝提交和资源释放被明确
    拆开，使 GPU/CPU Block 在异步传输完成前始终保持有效。
    """
    ...
    def prepare_swap_out(self, gpu_cache, request_id):
        """预留 CPU Block，但不发起拷贝，也不释放 GPU Block。"""
    ...
    def complete_swap_out(self, gpu_cache, request_id):
        """D2H 完成后释放 GPU 源 Block。"""
        if request_id not in self.swap_tables:
            raise KeyError("请求没有待完成的换出: " + request_id)
        gpu_cache.release(request_id)

    def prepare_swap_in(self, gpu_cache, request_id):
        """预留并登记 GPU 目标 Block，但不发起 H2D，也不释放 CPU Block。"""
        if request_id not in self.swap_tables:
            raise KeyError("请求不在 CPU 池中: " + request_id)
        cpu_ids = list(self.swap_tables[request_id])
    ...
    def complete_swap_in(self, request_id):
        """H2D 完成后释放 CPU 源 Block。"""
        if request_id not in self.swap_tables:
            raise KeyError("请求没有待完成的换入: " + request_id)
        for cpu_id in self.swap_tables[request_id]:
    ...
    def swap_out(self, gpu_cache, request_id):
        ...
        """
        stats = self.prepare_swap_out(gpu_cache, request_id)
        for gpu_id, cpu_id in zip(stats["gpu_ids"], stats["cpu_ids"]):
            self.blocks[cpu_id].copy_(gpu_cache.blocks[gpu_id])
        self.complete_swap_out(gpu_cache, request_id)
        stats.pop("gpu_ids")
        stats.pop("cpu_ids")
        return stats
    ...
    def swap_in(self, gpu_cache, request_id):
        ...
        """
        stats = self.prepare_swap_in(gpu_cache, request_id)
        for cpu_id, gpu_id in zip(stats["cpu_ids"], stats["gpu_ids"]):
            gpu_cache.blocks[gpu_id].copy_(self.blocks[cpu_id])
        self.complete_swap_in(request_id)
        stats.pop("gpu_ids")
        stats.pop("cpu_ids")
        return stats
