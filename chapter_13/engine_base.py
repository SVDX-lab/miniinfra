"""第 13 期独立执行底座：Prefill、Decode、计时与指标公共实现。
...
- sync baseline：默认计算 Stream 上同步换出/换入；
- async optimized：独立 Copy Stream 提交，Event 完成后才转移 Block 所有权；
- swapped 队列按接纳顺序 FCFS，恢复优先于新请求准入。

公开运行入口位于同目录 `engine.py`。本文件完整保留同步 Offloading 参考主循环，
供公共函数对照与教学阅读，但第 13 期实验不会调用它。
"""
...
def synchronize(device):
    """只等待计算 Stream，不能用设备级同步抹掉 Copy Stream 重叠。"""
    if device.type == "cuda":
...
def _synchronous_offloading_reference(
    model,
    request_specs,
