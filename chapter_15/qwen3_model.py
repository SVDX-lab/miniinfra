...
def load_handwritten_model(model_directory, device, dtype=None):
    ...
    expected_keys = set(model.state_dict()) - {"lm_head.weight"}
    loaded_keys = set()
    unexpected_keys = set()
    if weight_path.is_file():
        ...
        shards = [directory / name for name in sorted(set(weight_map.values()))]
        missing_shards = [str(path) for path in shards if not path.is_file()]
        if missing_shards:
            raise FileNotFoundError("缺少 Safetensors 分片: " + str(missing_shards))
    else:
        raise FileNotFoundError(
            "没有找到 model.safetensors 或 model.safetensors.index.json: "
            + str(directory)
        )
    for shard_path in shards:
        state_dict = load_file(str(shard_path), device="cpu")
