"""第 07 期完整 Prefill 与 Chunked Prefill 独立教学引擎。"""
...
def _finalize(states, events, config, peak_memory, cache, scheduler_ms_total):
    ordered = list(states)
...
def run_engine(
    ...
    """运行完整 Prefill 或 Chunked Prefill。"""
    if not request_specs:
        raise ValueError("request_specs 不能为空")
    for spec in request_specs:
        spec.validate()
    config = SchedulerConfig(mode, max_running_requests, token_budget)
    ...
    max_blocks = sum(math.ceil(length / block_size) for length in maximum_lengths)
